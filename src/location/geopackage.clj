(ns location.geopackage
  "Provides functions for processing data within geo-packages."
  (:use [location.macros])
...
(defn ^:private close-package
  "Closes the data stream from the geo-package and disposes its data store"
  [store data]
  (when (some? data) (.close data))
  (when (some? store) (.dispose store)))

;;(def valid-shapes
;;  "Fetches only geo-packages from the configured directory."
;;  (let [geo-package-path (str (cfg/get-config-first "geopackage.path"))
;;        dir (io/file geo-package-path)]
;;    (log/debug "Searching for geo-packages in" geo-package-path)
;;    (->> dir
;;      file-seq
...
;;
;;(def all-shapes
;;  "Returns geo-packages and products from the configured directory."
;;  (set/union valid-shapes cfg/products))
;;
;;(def ^:private blacklist-alias
;;  "Checks each alias for blacklisted files."
;;  (when-let [bl-set (set (cfg/get-config "geo-package.blacklist"))]
;;    (reduce-kv (fn [c k v]
...
;;
;;(def shapes
;;  "A set of all available geo-packages. This is the set difference between all files in the directory and the blacklisted files from configuration."
;;  (do
;;    (log/info "Available products: " (str/join ", " cfg/products))
;;    (log/info "Available geo-packages: " (str/join ", " valid-shapes))
;;    (when ((comp not empty?) (cfg/get-config "geo-package.blacklist"))
;;      (log/info "Blacklisted geo-packages: " (str/join ", " (cfg/get-config "geo-package.blacklist"))))
;;    (when-let [aliases blacklist-alias]
;;      (log/info "Aliases containing blacklisted files: " aliases))
;;    (set/difference all-shapes (cfg/get-config "geo-package.blacklist"))))
...
(defn ^:private get-geo-package
  "Returns the file path for the given geo-package."
  []
  (log/info "getting geo-package")
  (let [path (str (cfg/get-config-first "geo-package.path") "/" "lsd_prod_all.gpkg")
        file (-> path io/file)]
    (when (.exists file)
      (log/debug "found geo-package")
      file)))

(def get-geo-package
  "Returns the feature source data for the supplied geo-package. This function is memoized due to the small number of possible geo-packages."
  (memo/ttl get-geo-package :ttl/threshold (* (cfg/get-config "default.time.to.live") 1000)))

(defn ^:private get-features
  "Fetches the geo-package feature data using CQL."
  [polygon fltr]
  (when-let [store (FileDataStoreFinder/getDataStore get-geo-package)]
    (try
      [store
       (some-> store
...
(defn ^:private nil-attr
  "Gets the provided attribute from the provided geo-package data. If this yields a nil or empty value, nil is returned."
  [data attr]
  (util/nil-or-empty? (.getAttribute data attr)))

(defn additional-polygon-data
  "Adds any additional data for the polygon from the geo-package specified in the config."
  [polygon data]
...
(defn ^:private get-default-attribute
  "Fetches only the first attribute data from the given geo-package feature."
  [data]
  (when (.hasNext data)
    (as-> data $
          (.next $)
...
(defn ^:private get-attribute
  "Fetches the attribute data from the given geo-package feature."
  ...
  (try
    ...
    (catch Exception ex
      (log/error "Exception retrieving data from geo-package.")
      (log/error ex))))

;;(defn ^:private resolve-indexed-geo-package
;;  "Given a geo-package (or product) look up the attributes and using the first one,
;;  determine if this file contains the actual attributes or an index to the next geo-package.
;;  Returns the name of the geo-package with the actual attributes."
;;  [shape fltr]
;;  (if (false? (cfg/is-geo-package-index? shape))
;;    shape
;;    (if (or (nil? shape) (nil? fltr))
;;      shape
;;      (let [[store data] (get-features shape fltr)
;;            attr (some-> data get-default-attribute)]
;;        (close-package store data)
;;        (resolve-indexed-geo-package attr fltr)))))

(defn expand-product
  "Retrieves the geo-packages that make up the supplied product."
  [product]
...
(defn ^:private apply-key-mask
  "For the geo-packages which emit multiple attributes, zipmap them with the provided data."
  [product data]
  (if-let [mask (cfg/get-config (str "shapeflie.additional.attributes.mask." product))]
    (map (partial zipmap (map keyword mask)) (partition (count mask) data))
    data))

(defn ^:private get-polygon
  "Given a geocode, returns the requested polygon from the geo-package."
  ([fltr locale product]
...
(defn ^:private get-product
  ...
  (when-let [expanded (expand-product product)]
    ;;(map #(get-polygon fltr locale % (resolve-indexed-geo-package % fltr)) expanded)))
...
(defn ^:private get-key-attr
  "Returns the geo-package attribute which serves as the products key."
  [product]
  (or (cfg/get-config-first (str "geo-package.key.attribute." product))
      default-attr))
...
(defn get-geometries
  ...
  (when-let [expanded (expand-product product)]
    (let [filter-fn (if (cfg/is-line-string-product? product) touches-filter contains-filter)]
      ;;(map #(get-boundary (filter-fn geocode) % (resolve-indexed-geo-package % (filter-fn geocode))) expanded))))
